...
@router.get("/{policy_id}/history", response_model=PolicyHistory)
async def get_policy_history(
    policy_id: UUID,
    user: Annotated[UserOut, Depends(get_current_user)],
    session: Annotated[AsyncSession, Depends(get_tenant_db)],
) -> PolicyHistory:
    return await policy_service.get_history(session, user.id, policy_id)


@router.post("/{policy_id}/link", response_model=PolicyOut)
async def link_policy(
    policy_id: UUID,
    body: PolicyLinkRequest,
    user: Annotated[UserOut, Depends(get_current_user)],
    session: Annotated[AsyncSession, Depends(get_tenant_db)],
) -> PolicyOut:
    return await policy_service.link_policies(
        session, user.id, policy_id, body.peer_policy_id
    )


@router.delete("/{policy_id}/link", response_model=PolicyOut)
async def unlink_policy(
    policy_id: UUID,
    user: Annotated[UserOut, Depends(get_current_user)],
    session: Annotated[AsyncSession, Depends(get_tenant_db)],
) -> PolicyOut:
    return await policy_service.unlink_policy(session, user.id, policy_id)
